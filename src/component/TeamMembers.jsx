...
const activeMembers = team.filter((member) => {
  ...
  if (member.title === "UI UX Designer") {
    return filteredData.some((item) => item["Design By"] === member.sheetName);
  }
  return false;
});
...
{activeMembers.map((member, index) => {
  const isSelected =
    (member.title === "Web Developer" && filters.developer === member.sheetName) ||
    (member.title === "UI UX Designer" && filters.designer === member.sheetName);
